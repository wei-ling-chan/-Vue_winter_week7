    ...
    <table class="table mt-4">
      ...
      <tbody>
        <tr v-for="item in product" :key="item.id">
          ...
          <td>
            <div class="btn-group">
              <button type="button" class="btn btn-outline-primary btn-sm" @click="openModal('edit',item)" data-id="item.id">
                編輯
              </button>
  ...
  methods: {
    ...
    updateProduct (item) {
      ...
      if (!this.isNew) {
        ...
        api = `${VITE_APP_URL}/v2/api/wlc/admin/product/${this.template.id}`
      }
